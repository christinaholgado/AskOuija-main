...
const QuestionAsker = props => {
        ...
        while ((someQuestions.length < 3 ) && (counter.current < 150)) {
          ...
          if (dbQuestion.size > 0) {
            dbQuestion.forEach(q => {
              console.log('log for query1: ' + q.data()); 
  ...
  //^can put a property or state variable here. This line makes it rerun 




  //show the question list if no question has been asked
  //DOM gets rendered (rerun) every time you open the app, when there is a change to the state variables, when the parent is rendered, if you have passed a property 
  //DOM starts here:
