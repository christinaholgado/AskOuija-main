...
const Greeting = props => {
  ...
  const createGame = () => {
    generateGameCode()
    //the promise in save game is gameDoc.id and then it is followed by .then 
      .then(response => {
  ...
  const generateGameCode = () => {
    ...
    //Math.floor creates a random number between 0 and 1. ^need a random number between 0 and 25. If the random number generated is 5, the letter will be "F"
  ...
  const saveGame = newGameCode => {
    return firestore().collection("ao-games").add({
      //.add creates a new document in the "ao-games" collection with these objects
    ...
    .then(gameDoc => {
      return gameDoc.id;
      //gameDoc stores the whole document, gameDoc.id is the id that was autogenerated
      //the gameDoc.id is NOT the 4 letter game code 
    })
    .catch(err => {
